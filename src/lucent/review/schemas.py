...
A reviewer reads ONE finding's evidence and returns a narrow, validated review that says what
is actually going on at the cited sites and how much the finding's reading holds. It may not
change a brittle finding's fragility rating or write report prose beyond its explanation.
Malformed or uncertain output becomes ``needs_human`` rather than being dropped.
"""
...
class FindingReview(BaseModel):
    ...
    relevance: str = Field(
        default="", description="If the reader gave a goal or question, one sentence on how this "
                               "finding bears on it. Leave it empty if it does not bear on the goal.")
